...
const getBanner = () => {
    return `/*!
* Unilake v${pkg.version} (${pkg.homepage})
* @version ${pkg.version}
* @link ${pkg.homepage}
* Copyright ${year} The Unilake Authors
* Copyright ${year} Unilake B.V. Menno R. Hamburg
*/
`
}
...
gulp.task('sass', () => {
    return gulp
        .src(argv.withPlugins || BUILD ? `${srcDir}/Styles/!(_)*.scss` : `${srcDir}/Styles/+(tabler|demo).scss`)
        .pipe(debug())
        .pipe(sass({
            style: 'expanded',
            precision: 7,
...
gulp.task('build-internal', gulp.series('clean-dirs', 'sass', 'css-rtl', 'css-minify', 'js', gulp.parallel('js-demo', 'js-demo-theme'), 'mjs', 'copy-images', 'copy-libs', 'add-banner'))
