...
using webapp.Shared.Components.Tooltips;
using webapp.Shared.Forms;
using webapp.Shared.Icons;
using webapp.Shared.Infrastructure;

namespace webapp.Shared.Components.Buttons;
...
/// </summary>
public partial class DButton : ComponentBase, ICascadeEnabledComponent
{
	/// <summary>
	/// Application-wide defaults for <see cref="DButton"/> and derived components.
	/// </summary>
	...
	/// <summary>
	/// Position of the icon within the button. Default is <see cref="ButtonIconPlacement.Start" /> (configurable through <see cref="DButton.Defaults"/>).
	/// </summary>
	[Parameter] public ButtonIconPlacement? IconPlacement { get; set; }
	protected ButtonIconPlacement IconPlacementEffective => IconPlacement ?? GetSettings()?.IconPlacement ?? GetDefaults()?.IconPlacement ?? throw new InvalidOperationException(nameof(IconPlacement) + " default for " + nameof(DButton) + " has to be set.");

	/// <summary>
	/// Bootstrap button style - theme color.<br />
	/// Default is taken from <see cref="DButton.Defaults"/> (<see cref="ThemeColor.None"/> if not customized).
	/// </summary>
	[Parameter] public ThemeColor? Color { get; set; }
	protected ThemeColor ColorEffective => Color ?? GetSettings()?.Color ?? GetDefaults().Color ?? throw new InvalidOperationException(nameof(Color) + " default for " + nameof(DButton) + " has to be set.");

	/// <summary>
	/// Button size. Default is <see cref="ButtonSize.Regular"/>.
	/// </summary>
	[Parameter] public ButtonSize? Size { get; set; }
	protected ButtonSize SizeEffective => Size ?? GetSettings()?.Size ?? GetDefaults().Size ?? throw new InvalidOperationException(nameof(Size) + " default for " + nameof(DButton) + " has to be set.");

	/// <summary>
	/// <see href="https://getbootstrap.com/docs/5.2/components/buttons/#outline-buttons">Bootstrap "outline" button</see> style.
	/// </summary>
	[Parameter] public bool? Outline { get; set; }
	protected bool OutlineEffective => Outline ?? GetSettings()?.Outline ?? GetDefaults().Outline ?? throw new InvalidOperationException(nameof(Outline) + " default for " + nameof(DButton) + " has to be set.");
	...
	/// Leave <c>null</c> if you want automated spinner when any of the <see cref="DButton.OnClick"/> handlers is running.
	...
	protected string? GetColorCssClass()
	{
		if (OutlineEffective)
		{
			return ColorEffective switch
			{
				...
				_ => throw new InvalidOperationException($"Unknown {nameof(DButton)} color {ColorEffective:g}.")
			};
		}
		return ColorEffective switch
		{
	...
	protected string? GetSizeCssClass()
	{
		return SizeEffective switch
		{
			...
			_ => throw new InvalidOperationException($"Unknown {nameof(DButton)} {nameof(Size)}: {SizeEffective}.")
		};
	}
	...
	{
		// #209 [DButton] Tooltip does not hide when the button opens HxModal
