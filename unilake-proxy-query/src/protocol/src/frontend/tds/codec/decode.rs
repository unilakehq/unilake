...
fn read_string(src: &mut BytesMut, length: usize) -> Result<String> {
    if length > 0 {
        ...
        let result = String::from_utf16(&utf16_data).map_err(|err| {
            ErrorCode::TdsProtocolFailedToDecodeVarchar(format!(
                "Failed to decode varchar: {}",
                err
            ))
        })?;
